...
inputs = {
  nrql_alert_condition_name = "Category Search Latency"
  nrql_alert_condition_description = "Category Search Latency"
  nrql_alert_condition_policy_ids = []
  nrql_alert_condition_enabled = true
  nrql_alert_condition_query = "SELECT percentile(duration, 95) * 1000 as 'Latency' FROM Transaction where request.uri='/v1/categories/search' since 15 minutes ago"

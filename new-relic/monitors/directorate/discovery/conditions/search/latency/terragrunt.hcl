...
inputs = {
  nrql_alert_condition_name = "Product Search Latency"
  nrql_alert_condition_description = "Product Search Latency"
  nrql_alert_condition_policy_ids = []
  nrql_alert_condition_enabled = true
  nrql_alert_condition_query = "SELECT percentile(duration, 95) * 1000 as 'Latency' FROM Transaction where request.uri='/v1/internal/products' since 15 minutes ago"

...
inputs = {
  nrql_alert_condition_name = "Slug Success Rate"
  nrql_alert_condition_description = "Slug Success Rate"
  nrql_alert_condition_policy_ids = []
  nrql_alert_condition_enabled = true
  nrql_alert_condition_query = "SELECT percentage(count(*), WHERE httpResponseCode = '200') as SuccessRate FROM Transaction where request.uri='/v1/slugs' since 15 minutes ago"

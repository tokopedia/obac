...
inputs = {
  nrql_alert_condition_name = "Slug RPS"
  nrql_alert_condition_description = "Slug RPS"
  nrql_alert_condition_policy_ids = []
  nrql_alert_condition_enabled = true
  nrql_alert_condition_query = "SELECT rate(count(*), 1 second) AS 'RPS' FROM Transaction where request.uri='/v1/slugs' since 15 minutes ago"

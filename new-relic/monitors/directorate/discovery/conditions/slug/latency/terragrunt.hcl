...
inputs = {
  nrql_alert_condition_name = "Slug Latency"
  nrql_alert_condition_description = "Slug Latency"
  nrql_alert_condition_policy_ids = []
  nrql_alert_condition_enabled = true
  nrql_alert_condition_query = "SELECT percentile(duration, 95) * 1000 as 'Latency' FROM Transaction where request.uri='/v1/slugs' since 15 minutes ago"

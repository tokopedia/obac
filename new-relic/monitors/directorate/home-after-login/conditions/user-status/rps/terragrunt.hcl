...
inputs = {
  nrql_alert_condition_name = "User Status RPS"
  nrql_alert_condition_description = "User Status RPS"
  nrql_alert_condition_policy_ids = []
  nrql_alert_condition_enabled = true
  nrql_alert_condition_query = "SELECT rate(count(*), 1 second) AS 'RPS' FROM Transaction where request.uri='/api/v1/users/status' since 15 minutes ago"

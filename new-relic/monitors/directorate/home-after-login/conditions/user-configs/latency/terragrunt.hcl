...
inputs = {
  nrql_alert_condition_name = "User Configs Latency"
  nrql_alert_condition_description = "User Configs Latency"
  nrql_alert_condition_policy_ids = []
  nrql_alert_condition_enabled = true
  nrql_alert_condition_query = "SELECT percentile(duration, 95) * 1000 as 'Latency' FROM Transaction where request.uri='/api/v1/users/configs' since 15 minutes ago"

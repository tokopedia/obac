...
inputs = {
  nrql_alert_condition_name = "User Configs Success Rate"
  nrql_alert_condition_description = "User Configs Success Rate"
  nrql_alert_condition_policy_ids = []
  nrql_alert_condition_enabled = true
  nrql_alert_condition_query = "SELECT percentage(count(*), WHERE httpResponseCode = '200') as SuccessRate FROM Transaction where request.uri='/api/v1/users/configs' since 15 minutes ago"

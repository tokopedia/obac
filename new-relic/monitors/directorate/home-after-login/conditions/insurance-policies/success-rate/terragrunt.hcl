...
inputs = {
  nrql_alert_condition_name = "Insurance Policies Success Rate"
  nrql_alert_condition_description = "Insurance Policies Success Rate"
  nrql_alert_condition_policy_ids = []
  nrql_alert_condition_enabled = true
  nrql_alert_condition_query = "SELECT percentage(count(*), WHERE httpResponseCode = '200') as SuccessRate FROM Transaction where request.uri='/api/v1/insurance/policies' since 15 minutes ago"

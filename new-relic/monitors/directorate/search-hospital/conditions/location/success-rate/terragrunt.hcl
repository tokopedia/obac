...
inputs = {
  nrql_alert_condition_name = "Hospital Location Success Rate"
  nrql_alert_condition_description = "Hospital Location Success Rate"
  nrql_alert_condition_policy_ids = []
  nrql_alert_condition_enabled = true
  nrql_alert_condition_query = "SELECT percentage(count(*), WHERE httpResponseCode = '200') as SuccessRate FROM Transaction where request.uri='/api/rumah-sakit/v1/hospitals/locations' since 15 minutes ago"

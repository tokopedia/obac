...
inputs = {
  nrql_alert_condition_name = "Hospital Suggestion RPS"
  nrql_alert_condition_description = "Hospital Suggestion RPS"
  nrql_alert_condition_policy_ids = []
  nrql_alert_condition_enabled = true
  nrql_alert_condition_query = "SELECT rate(count(*), 1 second) AS 'RPS' FROM Transaction where request.uri='/api/rumah-sakit/v1/hospitals/suggestions' since 15 minutes ago"

...
inputs = {
  nrql_alert_condition_name = "Product detail Latency"
  nrql_alert_condition_description = "Product detail Latency"
  nrql_alert_condition_policy_ids = []
  nrql_alert_condition_enabled = true
  nrql_alert_condition_query = "SELECT percentile(duration, 95) * 1000 as 'Latency' FROM Transaction where request.uri like '/api/v1/buy-medicine/products/detail/%' since 15 minutes ago"

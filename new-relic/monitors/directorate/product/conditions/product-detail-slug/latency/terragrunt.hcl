...
inputs = {
  nrql_alert_condition_name = "Product Detail Slug Latency"
  nrql_alert_condition_description = "Product Detail Slug Latency"
  nrql_alert_condition_policy_ids = []
  nrql_alert_condition_enabled = true
  nrql_alert_condition_query = "SELECT percentile(duration, 95) * 1000 as 'Latency' FROM Transaction where request.uri like '/v1/internal/products/%/details' since 15 minutes ago"

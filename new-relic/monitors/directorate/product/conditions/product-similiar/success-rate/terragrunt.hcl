...
inputs = {
  nrql_alert_condition_name = "Product Similar Success Rate"
  nrql_alert_condition_description = "Product Similar Success Rate"
  nrql_alert_condition_policy_ids = []
  nrql_alert_condition_enabled = true
  nrql_alert_condition_query = "SELECT percentage(count(*), WHERE httpResponseCode = '200') as SuccessRate FROM Transaction where request.uri like '/api/v1/buy-medicine/products/similar-items/%' since 15 minutes ago"

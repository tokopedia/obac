...
inputs = {
  nrql_alert_condition_name = "Slug Multi Get All Latency"
  nrql_alert_condition_description = "Slug Multi Get All Latency"
  nrql_alert_condition_policy_ids = []
  nrql_alert_condition_enabled = true
  nrql_alert_condition_query = "SELECT percentile(duration, 95) * 1000 as 'Latency' FROM Transaction where request.uri='/v1/slugs/multiget/all' since 15 minutes ago"

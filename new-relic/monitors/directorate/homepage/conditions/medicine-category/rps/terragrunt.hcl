...
inputs = {
  nrql_alert_condition_name = "Buy medicine category RPS"
  nrql_alert_condition_description = "Buy medicine category RPS"
  nrql_alert_condition_policy_ids = []
  nrql_alert_condition_enabled = true
  nrql_alert_condition_query = "SELECT rate(count(*), 1 second) AS 'RPS' FROM Transaction where request.uri='/api/v1/buy-medicine/categories' since 15 minutes ago"

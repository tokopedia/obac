...
inputs = {
  nrql_alert_condition_name = "Doctor Availability RPS"
  nrql_alert_condition_description = "Doctor Availability RPS"
  nrql_alert_condition_policy_ids = []
  nrql_alert_condition_enabled = true
  nrql_alert_condition_query = "SELECT rate(count(*), 1 second) AS 'RPS' FROM Transaction where request.uri='/api/rumah-sakit/v1/hospitals/personnel/schedule/availability' since 15 minutes ago"

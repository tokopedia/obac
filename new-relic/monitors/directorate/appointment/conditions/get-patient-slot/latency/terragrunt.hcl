...
inputs = {
  nrql_alert_condition_name = "Get Patient Slot Latency"
  nrql_alert_condition_description = "Get Patient Slot Latency"
  nrql_alert_condition_policy_ids = []
  nrql_alert_condition_enabled = true
  nrql_alert_condition_query = "SELECT percentile(duration, 95) * 1000 as 'Latency' FROM Transaction where request.uri='/api/v1/appointments/get-by-patient-and-slot' since 15 minutes ago"
